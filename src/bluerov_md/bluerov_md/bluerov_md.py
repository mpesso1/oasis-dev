...
class MyNode(Node):
    def __init__(self, external, internal):
        ...
        # self.control_sub = self.create_subscription(String, 'key_controls', self.key_control_cb, 10)
        self.arm_disarm_sub = self.create_subscription(String, 'arm_disarm', self.arm_disarm_cb, 10)
        self.dvl_odom = self.create_subscription(Odometry, 'DVL_ODOM', self.dvl_odom_cb, 10)
        ...
        initp = [0,1,2,3,4,5]
        for i in range(6):
            # self.e.append(env(initp[i],5,.1, follow=True, external=external, internal=internal))
            # self.state.append(self.e[i].S(np.array([initp[i],0,0])))
            # self.state_.append(self.e[i].S(np.array([initp[i],0,0])))
            self.state.append(np.array([0,0]))
            self.state_.append(np.array([0,0]))
            self.a_error.append(np.zeros(num_of_actions))
        self.act = [0,0,0,0,0,0]

        #
        self.thruster_nominal = 1500
        self.thruster_power = 300
        self.thruster_total_positive = int(self.thruster_nominal + self.thruster_power)
        self.thruster_total_negative = int(self.thruster_nominal - self.thruster_power)

        print('Node constructed with state shape of: ', np.shape(self.state))

        # self.rovmav.manual_control_loop()

    def dvl_odom_cb(self,msg):
        # get position and veloity from dvl...
        self.state_[0] = np.array([msg.pose.pose.position.x, msg.twist.twist.linear.x])
        self.state_[1] = np.array([msg.pose.pose.position.y, msg.twist.twist.linear.y])
        self.state_[2] = np.array([msg.pose.pose.position.z, msg.twist.twist.linear.z])
        self.state_[3] = np.array([msg.pose.pose.orientation.x, msg.twist.twist.angular.x])
        self.state_[4] = np.array([msg.pose.pose.orientation.y, msg.twist.twist.angular.y])
        self.state_[5] = np.array([msg.pose.pose.orientation.z, msg.twist.twist.angular.z])
    ...
    def arm_disarm_cb(self,msg):
        ...
        if command == "d":
            ...
            self.rovmav.disarm_rov()

    def mode_cb(self,msg):
        command = msg.data

        if command == "p":
            self.rovmav.set_mode('POSHOLD')
        if command == "o":
            self.rovmav.set_mode("STABILIZE")
        if command == "[":
            self.rovmav.set_mode("MANUAL")

    def key_control_cb(self,msg):
        command = msg.data

        if command == "y":
            print("y")
            self.rovmav.set_rc_channel_pwm(self.rovmav.rc_channel['vz'],
                                           self.thruster_total_positive)
        if command == "h":
            print("y")

            self.rovmav.set_rc_channel_pwm(self.rovmav.rc_channel['vz'],
                                           self.thruster_total_negative)
        if command == "j":
            print("y")

            self.rovmav.set_rc_channel_pwm(self.rovmav.rc_channel['yaw'],
                                           self.thruster_total_positive)
        if command == "g":
            print("y")
        ...
        if command == "l":
            ...
            self.rovmav.set_rc_channel_pwm(self.rovmav.rc_channel['vx'],
                                           self.thruster_total_positive)
        if command == ".":
            print("y")
        ...
        if command == "/":
            ...
            self.rovmav.set_rc_channel_pwm(self.rovmav.rc_channel['vy'],
                                           self.thruster_total_positive)
        if command == ",":
            print("y")
        ...
        if command == "z":
            ...
            self.rovmav.set_rc_channel_pwm(self.rovmav.rc_channel['pitch'],
                                           self.thruster_total_positive)
        if command == "x":
            print("y")
        ...
        if command == "c":
            ...
            self.rovmav.set_rc_channel_pwm(self.rovmav.rc_channel['roll'],
                                           self.thruster_total_positive)
        if command == "v":
            print("y")
    ...
    def call_environment(self):
        ...
        # self.rovmav.set_rc_channel_pwm(5,1650)
        # self.rovmav.keyboard_controlls()
        # print('mason')
        # print('mason')
        ...
        for i in range(6):
            ...
            if i == 0 or i == 1 or i == 2 or i == 5:
                ...
                # send thrust
                thrust =  (self.act[i]+self.a_error[i][a_idx])
                # print("action: ", self.act[i], "  error: ", self.a_error[i][a_idx])
                if i==0:
                    a_idx_next = int(self.act[i+1]+1)
                    thrustx = thrust
                    thrusty =  (self.act[i+1]+self.a_error[i+1][a_idx_next])
                    thrust = thrustx*np.cos(self.state_[5][0]) - thrusty*np.sin(self.state_[5][0])
                    pid_thrust[0] = thrust
                    # self.rovmav.keyboard_controlls(pid_thrust_x=thrust)
                elif i==1:
                    a_idx_old = int(self.act[i-1]+1)
                    thrusty= thrust
                    thrustx =  (self.act[i-1]+self.a_error[i-1][a_idx_old])
                    thrust = thrustx*np.sin(self.state_[5][0]) + thrusty*np.cos(self.state_[5][0])
                    pid_thrust[1] = thrust
                    # self.rovmav.keyboard_controlls(pid_thrust_y=thrust)
                elif i==2:
                    pid_thrust[2] = thrust
                    # self.rovmav.keyboard_controlls(pid_thrust_z=thrust)
                elif i==5:
                    pid_thrust[3] = thrust
                    # self.rovmav.keyboard_controlls(pid_thrust_yaw=thrust)

                # # ****
                # # self.rovmav.set_rc_channel_pwm(int(self.map_id(i)),int(thrust)) #int(self.map_id(i))
                # self.rovmav.keyboard_controlls(pid_thrust_x=thrust)
        ...
        print(pid_thrust)
        # print(self.state)
        # self.rovmav.keyboard_controlls(pid_thrust[0],pid_thrust[1],pid_thrust[2])
        self.rovmav.keyboard_controlls()
