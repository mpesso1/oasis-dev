...
class PathPlanner {
    ...
    public:
        ...
        PathPlanner() : init_pose(Eigen::ArrayXXf::Zero(1,DOF)), 
                        init_vel(Eigen::ArrayXXf::Zero(1,DOF)),
                        init_acel(Eigen::ArrayXXf::Zero(1,DOF)),
                        goal_pose(Eigen::ArrayXXf::Zero(1,DOF)),
                        goal_vel(Eigen::ArrayXXf::Zero(1,DOF)),
                        goal_acel(Eigen::ArrayXXf::Zero(1,DOF))
        {
          float inf  = std::numeric_limits<float>::infinity();
          objects.push_back(Eigen::Matrix<float, 1, 3>{inf,inf,inf});
        };
