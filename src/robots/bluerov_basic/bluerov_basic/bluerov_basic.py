...
class MyNode(Node):
    ...
    def dvl_odom_cb(self,msg):
        linear_pos = self.rovmav.view_message('LOCAL_POSITION_NED',returndata=True,display_output=False)
        angular_pose = self.rovmav.view_message('ATTITUDE',returndata=True,display_output=False)
        # print(linear_pos)
        # print('----')
        # print(angular_pose)
        ...
        data = Odometry()
        if linear_pos != None:
            xin = linear_pos["x"] - self.xoffset
            yin = linear_pos['y'] - self.yoffset
            xout = xin*np.cos(self.yaw) - yin*np.sin(self.yaw)
            yout = xin*np.sin(self.yaw) + yin*np.cos(self.yaw)
            data.pose.pose.position.x = xin # float(msg.pose.pose.position.x)
            self.cx = linear_pos["x"]
            data.pose.pose.position.y = yin # float(msg.pose.pose.position.y)
            self.cy = linear_pos["y"]
            data.pose.pose.position.z = linear_pos['z'] - self.zoffset # float(msg.pose.pose.position.z)
            self.cz = linear_pos["z"]
            data.twist.twist.linear.x = linear_pos['vx'] #float(msg.twist.twist.linear.x)
            data.twist.twist.linear.y = linear_pos['vy'] #float(msg.twist.twist.linear.y)
            data.twist.twist.linear.z = linear_pos['vz'] #float(msg.twist.twist.linear.z)
    
        if angular_pose != None:
            data.pose.pose.orientation.x = angular_pose['roll'] #float(msg.pose.pose.orientation.x)
            data.pose.pose.orientation.y = angular_pose['pitch'] #float(msg.pose.pose.orientation.y)
            # if angular_pose['yaw'] != None:
            #     pass
            #     # data.pose.pose.orientation.z = angular_pose['yaw'] #float(msg.pose.pose.orientation.z)
            #     # self.yaw = angular_pose['yaw'] #float(msg.pose.pose.orientation.z)
            yaw = angular_pose['yaw']
            # if (self.lastya - yaw) > np.pi: # from 6 to 0
            #     yaw += 2*np.pi
            # elif (self.lastya - yaw) < -np.pi: # from 0 to 6
            #     yaw -= 2*np.pi

            data.pose.pose.orientation.w = yaw # stay negative
            if (yaw != abs(yaw)):
                yaw += 2*np.pi
            ...
            # elif (self.lastya - yaw) < -np.pi: # from 0 to 6
            #     yaw -= 2*np.pi
            data.pose.pose.orientation.z = yaw # not negative
            ...
            #     yaw -= 2*np.pi
            print(yaw)
            # data.pose.pose.orientation.z = yaw
            self.lastya = yaw
            self.yaw = yaw

            data.twist.twist.angular.x = angular_pose['rollspeed'] #float(msg.twist.twist.angular.x)
            data.twist.twist.angular.y = angular_pose['pitchspeed'] #float(msg.twist.twist.angular.y)
            data.twist.twist.angular.z = angular_pose['yawspeed'] #float(msg.twist.twist.angular.z)
    ...
    def mode_cb(self,msg):
        ...
        if command == "=":
            self.xoffset = self.cx
            self.yoffset = self.cy
            self.zoffset = self.cz

    # main loop... send over data on new pose estimate
    def timer_callback(self):
        
        # print(self.act)
        thrustx = self.act[0]*np.cos(self.yaw) + self.act[1]*np.sin(self.yaw)
        thrusty = -self.act[0]*np.sin(self.yaw) + self.act[1]*np.cos(self.yaw)
        # self.rovmav.keyboard_controlls(thrustx,thrusty,self.act[2],self.act[5])
        self.rovmav.keyboard_controlls(thrustx, thrusty, -self.act[2], self.act[5])
        # self.rovmav.keyboard_controlls(pid_thrust_z=-self.act[2], pid_thrust_yaw=self.act[5]) #,self.act[5])
    ...
    def listener_callback(self, msg):
        ...
        # if (sum(self.act) == 0) and self.mode == 0:
        #     self.rovmav.set_mode('POSHOLD')
        #     self.mode = 1
        # elif (sum(self.act) !=0) and self.mode == 1:
        #     self.rovmav.set_mode('STABILIZE')
        #     self.mode = 0
